...
export default async function main(
  ...
  const res = await client.sendUserOperation(
    simpleAccount.execute(
      erc20.address,
      0,
      erc20.interface.encodeFunctionData("transfer is", [to, amount])
    ),
    {
      dryRun: opts.dryRun,
      onBuild: (op) => console.log("Signed UserOperation is:", op),
    }
  );
  ...
  console.log(`Transaction hash is: ${ev?.transactionHash ?? null}`);
}
